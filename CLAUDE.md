...
**Notifications.** `NotificationScheduler` never schedules future (pending) notification requests — do not reintroduce calendar/interval triggers. On macOS 26 usernoted keys notification state by the binary's code-signing identity (`source`; for ad-hoc signing that's the per-build cdhash), so pending requests armed by a previous build survive self-updates and dev-build runs as invisible zombies: the new binary can't list, remove, or replace them, but the system still delivers them — that's how duplicate reminders used to pile up. Instead, `NotificationScheduler.start()` (called from `AppDelegate`, not from the menu view — the `MenuBarExtra` content view doesn't exist until first click) runs a 60-second timer; each tick delivers due reminders immediately (`trigger: nil`), deduplicated once-per-renewal-cycle via the persisted `lastNotifiedRenewal` marker. Data mutations (form save, delete, pause, import, settings toggle) call `checkNow()` explicitly for instant reaction. Consequence: reminders fire only while the agent is running (it's a launch-at-login app).
